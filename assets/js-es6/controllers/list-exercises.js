...
export async function renderizarListaExercicios(onNavigate) {
  const container = document.getElementById("lista-exercicios-container");
  const template = document.querySelector(".template-exercices-item");

  const btnCriar = document.getElementById("btn-create-exercise");

  if (btnCriar) {
    // Remove onclick antigo do HTML para não dar erro
    btnCriar.removeAttribute("onclick");

    btnCriar.onclick = () => {
      localStorage.removeItem("editExerciseId");
      if (onNavigate) onNavigate("exercisesAddEdit");
    };
  }
      ...
      btnDelete.onclick = async () => {
        ...
        if (confirmacao) {
          try {
            ...
            renderizarListaExercicios(onNavigate);
          } catch (err) {
